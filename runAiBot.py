...
from config.secrets import use_AI, use_AI_for_resume, username, password
...
def upload_optimized_resume(modal: WebElement, resume_path: str, job_description: str, job_id: str) -> tuple[bool, str]:
    try:
        ...
        return True, os.path.basename(optimized_resume_path)
    except Exception as e:
        print_lg(f"Failed to upload optimized resume for Job ID: {job_id}!", e)
...
def run_wrapper():
    while True:
        try:
            ...
            main()
            print_lg("Bot finished running. Waiting for 10 seconds before restarting...")
            sleep(10)
        except Exception as e:
